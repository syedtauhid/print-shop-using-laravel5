        ...
        <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
            <div class="white-box">
                ...
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation" class="active"><a ><span class="visible-xs"><i class="ti-user"></i></span> <span class="hidden-xs">Press Info</span></a></li>
                    ...
                    <div role="tabpanel" class="tab-pane active">
                                            ...
                                            @foreach($data as $key=>$item)
                                                <tr>
                                                    ...
                                                    <td class="text-nowrap">
                                                        ...
                                                        @if($key!=0)
                                                            @php
                                                                if(in_array($item->id,$priceDepedencyList)){
                                                                    $message = 'Your price map depend on this field. If you delete this field you need to set your price map from beginning.Do you want to delete this?';
                                                                }else{
                                                                    $message = 'Do you want to delete this?';
                                                                }
                                                            @endphp
                                                            <a class="btn btn-danger btn-circle deletePressField" href="{{route('admin.category.press-info.delete',$item->id)}}" data-message="{{$message}}" data-toggle="tooltip" data-original-title="delete">
                        ...
                        <div class="row">
                            <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12 well">
                                <div id="msg_press_info_form" class="alert alert-info  alert-dismissible">
                                    Add new Field
                                </div>
                                <form class="" id="add_press_info" method="post" action="{{route('admin.category.press-info',[$id])}}">
                                    {!! Form::token() !!}
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="placeholder" name="placeholder" required>
                                        </div>
                                    </div>
...
@section('page-specific-js')
    <script>
        var ERROR_MESSAGES = [
            "If you place a <strong> Label </strong> make sure <strong> placeholder </strong> name is same as <strong>Label</strong>",
            "<strong> Name </strong> is required",
            "please select a <strong>field type</strong>",
            "Special character's are not allowed. Remove <strong>"+'"'+"</strong>"
        ];
        $(document).ready(function () {
            ...
            $(document).on('click','.edit-button',function () {
                ...
                var url = '{!! route('admin.category.press-info.update',[':categoryId',':uploadInfoId']) !!}';
            ...
            $( "#add_press_info" ).submit(function( event ) {
                var label = $("#label").val();
                var placeholder = $("#placeholder").val();
                var select_type = $("#field-type").val();
                if (!isEmpty(label) && label !== placeholder){
                    $("#msg_press_info_form").removeClass("alert-info").addClass('alert-danger');
                    $("#msg_press_info_form").html(ERROR_MESSAGES[0]);
                    event.preventDefault();
                } else if (isEmpty(select_type)){
                    $("#msg_press_info_form").removeClass("alert-info").addClass('alert-danger');
                    $("#msg_press_info_form").html(ERROR_MESSAGES[2]);
                    event.preventDefault();
                } else if (isEmpty(placeholder)) {
                    $("#msg_press_info_form").removeClass("alert-info").addClass('alert-danger');
                    $("#msg_press_info_form").html(ERROR_MESSAGES[1]);
                    event.preventDefault();
                }

                if (!isValid(placeholder) || !isValid(label)){
                    $("#msg_press_info_form").removeClass("alert-info").addClass('alert-danger');
                    $("#msg_press_info_form").html(ERROR_MESSAGES[3]);
                    event.preventDefault();
                }
            });
        });
    ...
    <script>
        $(document).ready(function () {
            $(document).on('click','.deletePressField',function (e) {
                e.preventDefault();
                var message = $(this).data('message');
                warnBeforeChangeStatus($(this).attr('href'),message);
            });
        });
        function warnBeforeChangeStatus(deleteUrl,message) {
            swal({
                        title: "Are you sure?",
                        text: message,
                        type: "success",
